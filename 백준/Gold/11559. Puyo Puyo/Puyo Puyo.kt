    ...
    while (true) {
        ...
        for (y in (n - 1) downTo 0) {
            for (x in 0 until m) {
                if (map[y][x] != '.') {
                    queue.clear()
                    val bubbleCnt = dfs(y, x, map, visited, 1, map[y][x])
...
fun dfs(y: Int, x: Int, map: Array<CharArray>, visited: Array<BooleanArray>, cnt: Int, color: Char): Int {
    var count = cnt
    visited[y][x] = true
    queue += (y to x)
    ...
    for (i in 0..3) {
        ...
        if (ny in 0 until n && nx in 0 until m) {
            if (visited[ny][nx].not() && map[ny][nx] == color) {
                count = dfs(ny, nx, map, visited, count + 1, color)
            }
        }
    }
    return count
